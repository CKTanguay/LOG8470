/*** Generated by Spin Version 4.0.4 -- 12 April 2003 ***/
/*** From source: pan_in ***/
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 1:	/* :init: */
		...
		logval(":init::Ini[0]", ((P1 *)pptr(h))->Ini[0]);
		logval(":init::Ini[1]", ((P1 *)pptr(h))->Ini[1]);
		logval(":init::Ini[2]", ((P1 *)pptr(h))->Ini[2]);
		logval(":init::Ini[3]", ((P1 *)pptr(h))->Ini[3]);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
		break;
	case 0:	/* mutualExclusion */
		((P0 *)pptr(h))->_t = 0;
		((P0 *)pptr(h))->_p = 65; reached0[65]=1;
		/* params: */
		((P0 *)pptr(h))->index = par0;
		/* locals: */
		((P0 *)pptr(h))->counter = 0;
#ifdef VAR_RANGES
		logval("mutualExclusion:index", ((P0 *)pptr(h))->index);
		logval("mutualExclusion:counter", ((P0 *)pptr(h))->counter);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
		...
		now.flag[2] = 0;
		now.flag[3] = 0;
		now.procEnSectionCritique = 0;
#ifdef VAR_RANGES
		logval("I", now.I);
		logval("flag[0]", now.flag[0]);
		logval("flag[1]", now.flag[1]);
		logval("flag[2]", now.flag[2]);
		logval("flag[3]", now.flag[3]);
		logval("procEnSectionCritique", now.procEnSectionCritique);
#endif
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   I:	%d\n", now.I);
	printf("	byte   flag[0]:	%d\n", now.flag[0]);
	printf("	byte   flag[1]:	%d\n", now.flag[1]);
	printf("	byte   flag[2]:	%d\n", now.flag[2]);
	printf("	byte   flag[3]:	%d\n", now.flag[3]);
	printf("	byte   procEnSectionCritique:	%d\n", now.procEnSectionCritique);
}
void
...
{	/* int i; */
	...
	printf("	byte   Ini[3]:	%d\n", ((P1 *)pptr(pid))->Ini[3]);
		break;
	case 0:
		printf("local vars proc %d (mutualExclusion):\n", pid);
	printf("	byte   index:	%d\n", ((P0 *)pptr(pid))->index);
